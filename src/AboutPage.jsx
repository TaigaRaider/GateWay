export const SectionAbout = () => {
  const roles = [
    {
      title: "Software Engineer",
      desc: "Full-cycle development from conception to deployment. Systems thinking in every layer.",
    },
    {
      title: "Frontend",
      desc: "Responsive, accessible interfaces built with modern React and design-system discipline.",
    },
    {
      title: "Backend",
      desc: "Scalable APIs, event-driven architecture, and data pipelines that never skip a beat.",
    },
    {
      title: "DevOps",
      desc: "CI/CD, container orchestration, and infrastructure as code — automation as a mindset.",
    },
    {
      title: "Python",
      desc: "Scripting, data processing, and backend services written with clarity and purpose.",
    },
    {
      title: "Architect",
      desc: "System design that balances trade-offs between performance, cost, and maintainability.",
    },
  ];

  return (
    <section className="section" id="about">
      <div className="persona-section">
        <div>
          <span className="eyebrow">Profile</span>
          <h2 className="section-title">
            One<br />engineer<br />many roles
          </h2>
        </div>
        <div className="persona-grid">
          {roles.map((role, i) => (
            <div className="persona-card" key={i}>
              <svg className="persona-avatar" viewBox="0 0 40 40" preserveAspectRatio="xMidYMid meet">
                <rect width="40" height="40" fill="var(--placeholder-bg)" />
                <circle cx="20" cy="16" r="6" fill="none" stroke="var(--border-color)" strokeWidth="1.5"/>
                <path d="M10 32c0-5.523 4.477-10 10-10s10 4.477 10 10" fill="none" stroke="var(--border-color)" strokeWidth="1.5"/>
              </svg>
              <h4>{role.title}</h4>
              <p>{role.desc}</p>
              <div className="persona-socials">
                <a href="https://www.linkedin.com/in/adetunjiojo" target="_blank" rel="noopener noreferrer">
                  <svg viewBox="0 0 24 24" width="16" height="16" fill="var(--primary-text-color)">
                    <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
                  </svg>
                </a>
                <a href="mailto:adetunji.timothy@gmail.com">
                  <svg viewBox="0 0 24 24" width="16" height="16" fill="var(--primary-text-color)">
                    <path d="M24 5.457v13.909c0 .904-.732 1.636-1.636 1.636h-3.819V11.73L12 16.64l-6.545-4.91v9.273H1.636A1.636 1.636 0 010 19.366V5.457c0-2.023 2.309-3.178 3.927-1.964L5.455 4.64 12 9.548l6.545-4.91 1.528-1.145C21.691 2.28 24 3.434 24 5.457z"/>
                  </svg>
                </a>
                <a href="https://github.com/TaigaRaider" target="_blank" rel="noopener noreferrer">
                  <svg viewBox="0 0 24 24" width="16" height="16" fill="var(--primary-text-color)">
                    <path d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12"/>
                  </svg>
                </a>
              </div>
            </div>
          ))}
        </div>
      </div>
      <div className="cta-banner" id="contact">
        <h2>Let's build something<br />that holds weight</h2>
        <p>
          Whether you need a full platform or a precise component, I work best
          on problems that demand both depth and care.
        </p>
        <a href="mailto:adetunji.timothy@gmail.com" className="btn btn-primary">Start now</a>
      </div>
    </section>
  );
};